...
int Game::initSDL() {
  ...
  mGameWindow = SDL_CreateWindow("Dino 2D", SDL_WINDOWPOS_CENTERED,
                                 SDL_WINDOWPOS_CENTERED, mSCREEN_WIDTH,
                                 mSCREEN_HEIGHT, SDL_WINDOW_SHOWN);
  if (mGameWindow == nullptr) {
    std::cerr
        << "[Error] Game::initSDL(): SDL_CreateWindow() failed!\nDetails: "
        << SDL_GetError() << "\n";
    return -1;
  }
  ...
  if (mGameRenderer == nullptr) {
    std::cerr
        << "[Error] Game::initSDL(): SDL_CreateRenderer() failed!\nDetails: "
        << SDL_GetError() << "\n";
    return -1;
  }
...
void Game::showTitleScreen() {
  ...
  mTextManager.drawText_Static("DINO SAUR", CENTERED, Game::mSCREEN_HEIGHT / 5,
                               80, 150, INSTANT);
  mTextManager.drawText_Static("a 2D remake of the classic Chrome dinosaur "
                               "game\nby wldfngrs; https://github.com/wldfngrs",
                               CENTERED, 628, 12, 26, INSTANT);
  ...
  while (mInLobby) {
    ...
    if (subtitleIsVisible) {
      mTextManager.drawText_Static("press [SPACE] to start", CENTERED, 360, 18,
                                   40, INSTANT);
    }
...
void Game::showGameOverScreen() {
  ...
  while (mInLobby) {
    ...
    mTextManager.drawText_Static_NonStatic(
        "HIGH SCORE: ", std::to_string(Score::getHighScore()), CENTERED, 40, 24,
        40);
    mTextManager.drawText_Static_NonStatic(
        "CURRENT SCORE: ", std::to_string(Score::getCurrentScore()), CENTERED,
        90, 24, 40);

    mObstacleManager.updateGameOverAnimation();

    mTextManager.drawText_Static(Game::mGameOverMessage, CENTERED, 400, 24,
                                 Game::mSCREEN_HEIGHT / 14, INSTANT);

    if (subtitleIsVisible) {
      mTextManager.drawText_Static(
          "press [SPACE] to run again, [ALT + F4] to quit...", CENTERED, 600,
          18, 40, INSTANT);
    }
...
void Game::initNonDinoEntities() {
  background.addComponent<TransformComponent>(0, 0, Game::mSCREEN_WIDTH,
                                              Game::mSCREEN_HEIGHT - 80);
  background.addComponent<SpriteComponent>(
      "Assets/textures/BackgroundSheet.png", std::make_unique<Background>(), 0,
      64, 96, 64);

  celestialBody.addComponent<TransformComponent>(Game::mSCREEN_WIDTH, 150, 139,
                                                 130);
  celestialBody.addComponent<SpriteComponent>("Assets/textures/Moon.png",
...
void Game::resetNonDinoEntities() {
  TransformComponent *transformComponent =
      &celestialBody.getComponent<TransformComponent>();
  transformComponent->mPosition.x = static_cast<float>(Game::mSCREEN_WIDTH);

  SpriteComponent *spriteComponent =
      &background.getComponent<SpriteComponent>();
...
void Game::initDinoEntity() {
  mDino.addComponent<TransformComponent>(60, Game::mSCREEN_HEIGHT - 355, 273,
                                         275);
  mDino.addComponent<SpriteComponent>("Assets/textures/DinoSheet.png",
                                      std::make_unique<Dino>(), 0, 0, 32, 32);
...
void Game::initObstacles() { mObstacleManager.init(); }

void Game::resetObstacles() { mObstacleManager.reset(); }
...
void Game::handleEvents() {
  ...
  case SDL_KEYDOWN:
    if (mInLobby && (Game::mEvent.key.keysym.sym == SDLK_SPACE ||
                     Game::mEvent.key.keysym.sym == SDLK_UP)) {
      mInLobby = false;
...
bool Game::initializationDone() const { return mInitDone; }

bool Game::playerHasQuit() const { return mPlayerQuit; }

TextManager &Game::getTextManager() { return mTextManager; }
